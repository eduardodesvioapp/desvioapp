-- =============================================================
-- 🔧 MIGRATION: Fix AI Chat Response Pipeline
-- Corrige: filter_contact_info bloqueando IAs, processador ausente
-- =============================================================

-- 1. CORREÇÃO: filter_contact_info ignora mensagens de perfis IA
...
-- 2. Colunas extras em ai_chat_queue (idempotente)
...
-- 5. Processador principal da fila
...
-- 7. Agenda no pg_cron (somente se a extensão existir)
DO $cron$
DECLARE
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠️ pg_cron não está habilitado. O worker Node.js ou chamada manual é necessária.';
    RAISE NOTICE '   Execute manualmente: SELECT public.process_ai_chat_queue(5);';
    RAISE NOTICE '   Ou via cron do sistema: curl -X POST ...';
  END IF;
END
$cron$;

-- 8. Sincroniza chave Gemini do .env.local para public.secrets (se existir)
--    A chave será lida da variável de ambiente e inserida no banco
--    pelo script sync_secrets.cjs. Aqui garantimos que a tabela existe.
...
-- 9. Recarrega schema do PostgREST
